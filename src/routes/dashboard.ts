...
export async function handleDashboardAllowlistGet(
  _request: Request,
  ctx: ServerContext,
): Promise<Response> {
...
export async function handleDashboardAllowlistPost(
  request: Request,
  ctx: ServerContext,
): Promise<Response> {
...
function parseFilters(request: Request): DashboardFilters {
  ...
  const traceKind =
    traceKindRaw === "search-result-fetch" ||
    traceKindRaw === "direct-web-fetch" ||
    traceKindRaw === "unknown"
      ? traceKindRaw
      : undefined
