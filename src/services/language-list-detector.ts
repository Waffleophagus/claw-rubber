...
const LANGUAGE_CUE_RE = /\b(language|languages|idioma|idiomas|langue|lingua|sprache|язык|لغة|언어|言語)\b/iu;
...
export function detectLanguageList(text: string, extraNames: string[] = []): LanguageListDetection {
  ...
  const tokens = [...normalized.matchAll(TOKEN_RE)].map((match) => match[0]).filter((token) => token.length > 0);
  ...
  const hasStrongLanguageDensity = distinctMatchCount >= 4 && matchedTokenCount >= 5 && matchedRatio >= 0.45;
  const hasVeryStrongLanguageDensity = distinctMatchCount >= 8 && matchedTokenCount >= 8 && matchedRatio >= 0.35;
  const hasListSignals = listSignalCount >= 2 || matchedRatio >= 0.7 || hasCue;
  const isLanguageListLikely = (hasStrongLanguageDensity && hasListSignals) || hasVeryStrongLanguageDensity;
