...
const RULES: Rule[] = [
  {
    ...
    weight: 4,
    pattern:
      /(ignore|disregard|override)\b.{0,40}\b(previous|prior|all)\b.{0,30}\b(instruction|prompt|rule)/i,
    target: "normalized",
  },
  {
    id: "role_hijack",
    weight: 3,
    pattern:
      /\b(you are now|act as|pretend to be)\b.{0,40}\b(system|developer|administrator|root)\b/i,
    target: "normalized",
  },
  {
    ...
    weight: 4,
    pattern:
      /\b(show|reveal|print|leak|expose)\b.{0,30}\b(system prompt|developer message|hidden instructions?)\b/i,
    target: "normalized",
  },
  {
    ...
    weight: 5,
    pattern:
      /\b(api key|access token|secret|password|private key)\b.{0,30}\b(send|share|output|return|dump)\b/i,
    target: "normalized",
  },
  {
    id: "tool_abuse",
...
export function scorePromptInjection(
  content: string,
  options: ScorePromptInjectionOptions = {},
): InjectionScore {
  ...
  if (hasSuspiciousConfusableTokens) {
    if (!languageList?.isLanguageListLikely && hasHighRiskIntent) {
      ...
      evidence.push(
        ...collectConfusableEvidence(
          normalized.confusableAnalysis.suspiciousTokens,
          confusableWeight,
        ),
      )
    }
  ...
  const obfuscationSignals = allFlags.filter(
    (flag) =>
