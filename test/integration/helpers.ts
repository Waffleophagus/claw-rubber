...
function resolveIntegrationConfig(
  ...
  const webFetchTargetUrl = (
    env.CLAWRUBBER_INTEGRATION_WEB_FETCH_URL ?? fileConfig.webFetchTargetUrl
  ).trim()
  const envLanguageExtras = parseCsv(env.CLAWRUBBER_INTEGRATION_LANGUAGE_ALLOWLIST_EXTRA)
  const languageAllowlistExtras =
    envLanguageExtras.length > 0
      ? envLanguageExtras
      : fileConfig.languageAllowlistExtras
          .map((item) => item.trim())
...
function normalizeBaseUrl(raw: string): {
  ...
  if (!raw) {
    return {
      ...
      integrationDisabledReason:
        "set test/integration/config.ts baseUrl or CLAWRUBBER_INTEGRATION_BASE_URL",
    }
  }

  const withScheme = /^[a-zA-Z][a-zA-Z\d+\-.]*:\/\//.test(raw)
    ? raw
    : `${defaultSchemeForHost(raw)}://${raw}`
  try {
