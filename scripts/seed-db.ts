...
function jitteredTimestamp(start: number, spanMs: number, index: number, total: number, rng: () => number): number {
...
function main(): void {
  ...
  try {
    ...
    for (let i = 0; i < options.fetchEvents; i += 1) {
      ...
      if (decision === "block") {
        ...
        const allowedBy = maybe("language-exception", 0.16, rng) ?? maybe("domain-allowlist-bypass", 0.2, rng);
    ...
    upsertRuntimeAllowlist.run("developer.mozilla.org", "Manual exception for reference docs", now - 90 * 60 * 1000);
  ...
  const fetchCount = db.query(`SELECT COUNT(*) AS count FROM fetch_events`).get() as { count: number };
  const searchBlockCount = db.query(`SELECT COUNT(*) AS count FROM search_block_events`).get() as { count: number };
  const payloadCount = db.query(`SELECT COUNT(*) AS count FROM flagged_payloads`).get() as { count: number };
